...
class CodePushUpdateUtils {
     ...
     */
    func copyNecessaryFilesFromCurrentPackage(diffFile diffManifestFilePath: URL, currentPackagePath currentPackageFolderPath: URL,
        ...
        for file in diffManifest.deletedFiles {
            ...
            let fileToDelete = fileUtils.appendPathComponent(atBasePath: newPackageFolderPath, withComponent: subUrl.path)
     ...
     * Throws: Error due to IO exceptions
     */
    func findEntryPointInUpdateContents(atOrigin folderPath: URL, targetFile expectedFileName: String) throws -> URL? {
        ...
        for file in files {
            ...
            if (isDir.boolValue) {
                let mainBundlePathInSubFolder = try findEntryPointInUpdateContents(atOrigin: fullFilePath, targetFile: expectedFileName)
